# Release Sanity Checker (Go Implementation) ⚠️🤖🦾
...
# Run with all config files in a directory
go run . --directory ../examples
```
...
```bash
go build -o release-sanity-checker .
```
...
```bash
release-sanity-checker config.json --ignore-headers
```
